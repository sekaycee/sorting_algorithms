...
void sort_deck(deck_node_t **deck)
{
	...
	while (swap != 0)
	{
		...
		{
			c1 = get_val(curr->card->value) + 13 * curr->card->kind;
			c2 = get_val(curr->next->card->value) + 13 * curr->next->card->kind;
			if (c1 > c2)
			{
				swap_node(deck, curr);
				swap = 1;
		...
		{
			c1 = get_val(curr->card->value) + 13 * curr->card->kind;
			c2 = get_val(curr->prev->card->value) + 13 * curr->prev->card->kind;
			if (c1 < c2)
			{
